function DCM_spec_estim_crbl2(my_folder_path, dcm_id, init, nsub, estimate)
% =========================================================================
...
for subject = init:nsub
    ...
    if isfolder( fullfile(my_folder_path,name) )
        ...
        dcm_id_complete = horzcat(dcm_date,'_', dcm_id); %current date as ID
        s = struct();
        s.name       = dcm_id_complete;
        ...
        if estimate
            ...
            DCM = spm_dcm_estimate(horzcat('DCM_',dcm_id_complete,'.mat'))
        end
